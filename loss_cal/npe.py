...
def train_npe(
    ...
    **kwargs,
    ...
    inference_method = inference.SNPE_C(prior, density_estimator=density_estimator_fun, device=device.type)
    proposal = prior

    # Train for one round
    density_estimator = inference_method.append_simulations(theta_train, x_train, proposal=proposal).train(
        num_atoms=num_atoms,
